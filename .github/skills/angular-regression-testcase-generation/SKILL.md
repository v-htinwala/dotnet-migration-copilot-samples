---
name: angular-regression-testcase-generation
description: >
  Generates regression test cases for Angular codebases. Selects existing
  tests and generates new tests based on change-based, risk-based, and coverage-based
  qualification criteria. Supports Angular CLI projects with Jasmine+Karma or Jest
  (auto-detected from angular.json/package.json). Generates both unit-level (TestBed,
  jasmine.createSpyObj/jest.fn) and integration-level (HttpClientTestingModule,
  RouterTestingModule, Cypress component testing) regression tests. Supports
  user-supplied test data (fixtures, JSON seeds), configurable business criticality,
  ...
  Use when the user asks to generate regression tests, select regression test cases,
  prioritize tests based on code changes, or improve regression test coverage for
  Angular projects.
license: Apache-2.0
compatibility: >
  Best with VS Code Copilot or agents supporting the runSubagent tool for parallel
  processing. Falls back to sequential file-by-file processing on other
  platforms. Requires Python 3.8+ for helper scripts. Node.js 16+ and Angular CLI
  must be installed.
metadata:
  author: regression-testcase-generation
  version: "1.0"
allowed-tools: Bash(python:*) Bash(npx:*) Bash(ng:*) Bash(npm:*) Bash(yarn:*) Bash(pnpm:*) Bash(git:*) Read
---

# Angular Regression Test Case Generation

## When to Use This Skill

Activate this skill when the user wants to:
- Generate regression tests for an Angular codebase based on recent code changes
...
- Analyze the impact of changes across an Angular project (components, services, guards, interceptors, pipes, directives)
...
|---|---|---|---|
| **Codebase path** | Yes | Current workspace root | Angular project root (must contain `angular.json`) |
...
| **Test framework** | No | auto-detect | Jasmine+Karma or Jest (auto-detected from `angular.json` builder config / `package.json`) |
| **Mock library** | No | auto-detect | `jasmine.createSpyObj` (Jasmine) or `jest.fn`/`jest.mock` (Jest) |
| **Test data dir** | No | `src/test-data/` or `__fixtures__/` | Path to fixture files (JSON) for data-driven tests |
...
| **Test level** | No | `unit` | `unit` (TestBed) or `integration` (HttpClientTestingModule, RouterTestingModule, Cypress component testing) |
| **Coverage format** | No | `istanbul` | Istanbul coverage via Karma or Jest |
| **Business criticality map** | No | none | JSON mapping of file/path glob patterns to criticality levels (overrides path heuristics) |
...
2. **NEVER alter existing test methods.** When adding to an existing test file,
   only append new `describe`/`it` blocks — do not change existing ones.
3. **All file writes MUST target test files only.** Valid patterns:
   - `*.spec.ts` (Angular convention for unit tests)
   - `*.integration.spec.ts` (integration tests)
4. **Before writing any file**, verify the target path does NOT match a source
   file pattern (i.e., must end with `.spec.ts`).
5. **NEVER modify** `.ts`, `.html`, `.scss`, `.css`, `angular.json`, or `package.json` source files.
...
- Validate codebase path contains `angular.json` and Angular source files
- Record `git status` for integrity checks later
- Detect build system: `angular.json` (Angular CLI)
- Detect test framework: check `angular.json` for builder config (`@angular-devkit/build-angular:karma` vs `jest-builder` or equivalent); check `package.json` for `jest` dependency
...
- If `test_level` is `integration` or `both`, verify `@angular/common/http/testing` and `@angular/router/testing` are available
...
- Returns `changes.json`: list of changed `.ts`, `.html`, `.scss` files with diff stats
...
- Parses TypeScript `import {} from '...'` and `require()` statements
- Resolves imports via `tsconfig.json` `paths` aliases
- Detects `@NgModule` `imports`/`declarations`/`providers` arrays for Angular-specific dependency tracking
- Returns `impact.json`: changed files + all transitively impacted files
- Runs `scripts/list-existing-tests.py` to map impacted source files to existing `*.spec.ts` files
...
- Computes cyclomatic complexity (Angular-specific keywords: `subscribe`, `pipe`, `switchMap`, `?.`, `??`), change frequency, defect history, business criticality
- Path heuristics: `**/services/**` (high), `**/guards/**` (high), `**/interceptors/**` (high), `**/components/**` (medium), `**/pipes/**` (low), `**/models/**` (low)
...
- Each receives: TypeScript source file, component/service metadata, gap details, Angular testing reference, test framework (Jasmine/Jest), test level, matching fixtures, matching regression scenarios, requirement IDs
- For `test_level=unit`: generate `TestBed.configureTestingModule()` tests with `jasmine.createSpyObj()`/`jest.fn()` mocks
- For `test_level=integration`: generate tests using `HttpClientTestingModule`, `RouterTestingModule`, `NoopAnimationsModule`
- For `test_level=both`: generate separate unit and integration test blocks in `*.spec.ts` and `*.integration.spec.ts` files
- When user-provided fixture files match the target entity, generator loads fixture data via `import` or `fs.readFileSync`
...
- When requirement IDs are provided, generator adds `// REQ-xxx` comments and `fdescribe`/`fit` tagging conventions
- Write test files to the same directory as the source file, following Angular convention
- **Safety**: verify all writes target `*.spec.ts` files only
...
- Karma: `ng test --watch=false --browsers=ChromeHeadless --code-coverage`
...
- Returns test results + Istanbul coverage on changed code
...
6. For each coverage gap (one at a time):
   ...
   d. Write the test file as `*.spec.ts`
...
|---|---|---|
| `user.service.ts` | `user.service.spec.ts` | `user.service.integration.spec.ts` |
| `order.component.ts` | `order.component.spec.ts` | `order.component.integration.spec.ts` |
| `auth.guard.ts` | `auth.guard.spec.ts` | `auth.guard.integration.spec.ts` |
| `token.interceptor.ts` | `token.interceptor.spec.ts` | `token.interceptor.integration.spec.ts` |
  ...
  regression-tests:
    ...
    steps:
      ...
      - name: Run regression shard
        run: |
          python scripts/run-regression-tests.py . --test-runner karma \
...
|---|---|---|---|
| `analyze-changes.py` | Parse git diff for changed Angular files | `<codebase> [--ref HEAD~1]` | `changes.json` |
| `build-dependency-graph.py` | Build TypeScript import + NgModule dependency graph | `<codebase> --depth <N>` | `impact.json` |
| `calculate-risk-scores.py` | Compute risk per TypeScript file | `<files-json> --window 90 [--criticality-map <json>]` | `risk-scores.json` |
| `list-existing-tests.py` | Map Angular source files to spec files | `<module-path>` | `test-map.json` (includes `testType: unit&#124;integration`) |
...
| `run-regression-tests.py` | Execute selected tests via ng test/jest | `<module> --test-runner <karma&#124;jest> [--ci-mode] [--shard-index N --total-shards M] [--flaky-threshold N]` | `results.json` |
| `check-coverage.py` | Istanbul coverage gaps on changed code | `<report> --threshold <N>` | `coverage-gaps.json` |

## Language-Specific Reference

Load the Angular regression patterns reference when generating tests:

- [Angular Regression Patterns](references/regression-patterns.md)
