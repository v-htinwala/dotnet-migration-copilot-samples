#!/usr/bin/env python3
"""
calculate-risk-scores.py — Compute risk scores for Angular/TypeScript source files.
...
# Cyclomatic complexity keywords for TypeScript/Angular
COMPLEXITY_KEYWORDS = [
    r"\bif\b",
    r"\belse\s+if\b",
    r"\bfor\b",
...
# Business criticality path heuristics for Angular
CRITICALITY_HIGH = [
    r"service[s]?/",
    r"guard[s]?/",
    r"interceptor[s]?/",
...
def calculate_business_criticality(
    ...
    # Components get medium criticality by default
    if re.search(r"component[s]?/", normalized) or normalized.endswith(".component.ts"):
        return 0.5
...
def main():
    parser = argparse.ArgumentParser(
        description="Compute risk scores for Angular/TypeScript source files."
    )
    parser.add_argument(
        "files_json",
    ...
    parser.add_argument(
        "--criticality-map",
        help="Path to JSON file mapping file/path glob patterns to criticality levels",
    )
    ...
    # Extract file list from impact.json or changes.json format
    ...
    # Calculate raw metrics for each file
    ...
    # Calculate normalized scores and composite risk
