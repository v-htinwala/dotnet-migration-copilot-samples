---
name: csharp-regression-testcase-generation
description: >
  Generates regression test cases for C#/.NET codebases. Selects existing tests
  and generates new tests based on change-based, risk-based, and coverage-based
  qualification criteria. Supports dotnet CLI, solution/project structure,
  xUnit/NUnit, Moq, and Cobertura (coverlet) coverage. Generates both unit-level
  (Moq) and integration-level (WebApplicationFactory, TestServer, EF Core InMemory,
  Testcontainers) regression tests. Supports user-supplied test data (fixtures,
  SQL seeds, golden files), configurable business criticality, user-defined
  regression scenarios with requirement traceability, and CI/CD integration with
  artifact generation, test sharding, and flaky test detection. Use when the user
  asks to generate regression tests, select regression test cases, prioritize
  tests based on code changes, or improve regression test coverage for C# or
  .NET projects.
license: Apache-2.0
compatibility: >
  Best with VS Code Copilot or agents supporting the runSubagent tool for parallel
  multi-project processing. Falls back to sequential file-by-file processing on other
  platforms. Requires Python 3.8+ for helper scripts. .NET SDK 6.0+ required.
  dotnet CLI must be installed.
metadata:
  author: regression-testcase-generation
  version: "1.0"
allowed-tools: Bash(python:*) Bash(dotnet:*) Bash(git:*) Read
---

# C#/.NET Regression Test Case Generation

## When to Use This Skill

Activate this skill when the user wants to:
- Generate regression tests for a C#/.NET codebase based on recent code changes
...
- Analyze the impact of changes across a .NET solution with multiple projects
...
|---|---|---|---|
| **Codebase path** | Yes | Current workspace root | .NET solution/project root |
...
| **Mock library** | No | Moq | Mock library for generated tests |
| **Test framework** | No | xUnit | Auto-detected from .csproj (xUnit or NUnit) |
| **Test level** | No | `unit` | `unit`, `integration`, or `both`. Controls whether to generate mock-only tests, ASP.NET Core integration tests, or both |
| **Test data dir** | No | `TestData` | Path to fixture files (JSON, CSV) for data-driven tests |
| **SQL seeds** | No | none | List of SQL seed files to use with EF Core InMemory or Testcontainers |
...
| **Business criticality map** | No | none | JSON mapping of file/namespace glob patterns to criticality levels (overrides path heuristics) |
...
| **CI mode** | No | `false` | When `true`, generate CI artifacts (TRX, JSON manifest) in `report_dir` |
| **Report dir** | No | `./regression-reports` | Output directory for CI report artifacts |
| **Report formats** | No | `trx,json` | Artifact formats: `trx`, `html`, `json`, `markdown` |
...
1. **NEVER modify, rename, or refactor any existing source file.** Only create
   new files in test projects or append new test methods to existing test files.
...
3. **All file writes MUST target test projects only.** Valid patterns:
   - `*.Tests/*.cs`
   - `*.Test/*.cs`
   - `*.UnitTests/*.cs`
   - `*.IntegrationTests/*.cs`
4. **Before writing any file**, verify the target path is within a test project
   (contains `.Tests` or similar in the project name).
...
- Validate codebase path contains C# source files
- Record `git status` for integrity checks later
- Detect solution structure from `.sln` files
- Detect test framework from `.csproj` PackageReference (xUnit vs NUnit)
- Identify test projects by naming convention (`*.Tests`, `*.Test`, `*.UnitTests`, `*.IntegrationTests`)
...
- If `test_level` is `integration` or `both`, verify `Microsoft.AspNetCore.Mvc.Testing` is in test project dependencies; warn if Testcontainers dependency is missing
...
- Returns `changes.json`: list of changed `.cs` files with diff stats
- If user provided manual file list, skip git analysis and use that

#### Phase 3: Impact Analysis (Parallel per project)
- **change-analyzer** subagent runs `scripts/build-dependency-graph.py`
- Parses C# `using` directives and resolves via `.csproj` `<ProjectReference>` elements
...
#### Phase 4: Risk Scoring (Parallel per project)
...
- Each receives: C# source file, type summary, gap details, xUnit/Moq reference, test level, matching fixtures/seeds, matching regression scenarios, requirement IDs
- For `test_level=unit`: generate mock-based tests (default, same as before)
- For `test_level=integration`: generate `WebApplicationFactory`/`TestServer`/EF Core InMemory tests as appropriate — place in `*.IntegrationTests` project
- For `test_level=both`: generate separate unit tests in `*.Tests` and integration tests in `*.IntegrationTests` projects
- When user-provided fixture files match the target entity, generator uses `ClassData`/`MemberData` fixture loading or JSON deserialization instead of inline hardcoded data
- When regression scenarios target this file, generator creates nested classes per scenario with descriptive display names matching the scenario narrative
- When requirement IDs are provided, generator adds `[Trait("Requirement", "REQ-xxx")]` for traceability
- Write test files to the corresponding test project
- **Safety**: verify all writes target test projects only

#### Phase 7: Execution & Verification (Parallel per project)
- Spawn **regression-verifier** subagent per test project
- Runs `scripts/run-regression-tests.py` with selected + generated tests
- `dotnet test` with `--collect:"XPlat Code Coverage"`
- For `test_level=integration` or `both`: also run integration test projects (`*.IntegrationTests`)
- If `test_splitting` > 1, partition selected tests across shards using `--shard-index` and `--total-shards` flags
- Returns test results + Cobertura coverage on changed code
...
- If `ci_mode=true`: generate CI artifacts (TRX summary, JSON manifest) in `report_dir`

#### Phase 8: Report
- Run `git diff --name-only` — verify zero source file modifications
- Output final summary:
  - Changed files detected
  - Impact analysis: N files impacted across M projects
...
6. For each coverage gap (one at a time):
   ...
   d. Write the test file to the `*.Tests` project
...
per scenario with descriptive display names matching the scenario narrative.

### Requirement Traceability

When `requirement_ids` are provided (via user input, scenario file, or config):
- Each generated test class gets `[Trait("Requirement", "REQ-xxx")]` at the class level
- Each test method's display name includes the requirement ID: `[REQ-xxx]`
...
|---|---|---|
| `UserService.cs` | `UserServiceTests.cs` | `UserServiceIntegrationTests.cs` |
| `OrderController.cs` | `OrderControllerTests.cs` | `OrderControllerIntegrationTests.cs` |

## Multi-Project Solution Handling

### .NET Solution Structure
- Parse `.sln` file for `Project("...")` entries
- Each source project has a corresponding `*.Tests` project
- Tests run per-project: `dotnet test ProjectName.Tests/`
- Coverage per-project with coverlet: `dotnet test --collect:"XPlat Code Coverage"`
- Process order: projects with fewest `<ProjectReference>` dependencies first

### Project References
- Parse `.csproj` files for `<ProjectReference>` elements
- Cross-project dependencies are used to build the dependency graph
- Test project references source project for compilation
...
| File | Format | Purpose |
|---|---|---|
| `regression-results.trx` | TRX | CI dashboard integration (Azure DevOps, GitHub Actions) |
...
- dotnet test: uses `dotnet test --filter` to re-run individual failed tests
  ...
  regression-tests:
    ...
    steps:
      ...
      - name: Run regression shard
        run: |
          python scripts/run-regression-tests.py . --test-framework xunit \
      ...
      - uses: dorny/test-reporter@v1
        with:
          ...
          path: ./regression-reports/regression-results.trx
          reporter: dotnet-trx
```
    ...
    steps:
      ...
      - task: PublishTestResults@2
        inputs:
          ...
          testResultsFiles: ./regression-reports/regression-results.trx
...
|---|---|---|---|
| `analyze-changes.py` | Parse git diff for changed C# files | `<codebase> [--ref HEAD~1]` | `changes.json` |
| `build-dependency-graph.py` | Build C# using-directive dependency graph | `<codebase> --depth <N>` | `impact.json` |
| `calculate-risk-scores.py` | Compute risk per C# file | `<files-json> --window 90 [--criticality-map <json>]` | `risk-scores.json` |
| `list-existing-tests.py` | Map C# source files to test files | `<solution-path>` | `test-map.json` (includes `testType: unit&#124;integration`) |
...
| `run-regression-tests.py` | Execute selected tests via dotnet test | `<project> --test-framework <type> [--ci-mode] [--include-integration] [--shard-index N --total-shards M] [--flaky-threshold N]` | `results.json` |
| `check-coverage.py` | Cobertura coverage gaps on changed code | `<report> --threshold <N>` | `coverage-gaps.json` |

## Language-Specific Reference

Load the C# regression patterns reference when generating tests:

- [C# Regression Patterns](references/regression-patterns.md)
