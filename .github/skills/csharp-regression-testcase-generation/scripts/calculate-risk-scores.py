#!/usr/bin/env python3
"""
calculate-risk-scores.py — Compute risk scores for C# source files.
...
Calculates a composite risk score per file based on:
  ...
  - Business criticality (path heuristics)
"""
...
# Cyclomatic complexity keywords for C#
COMPLEXITY_KEYWORDS = [
    r"\bif\b",
    r"\belse\s+if\b",
    r"\bfor\b",
...
# Business criticality path heuristics for C#
CRITICALITY_HIGH = [
    r"[Cc]ontrollers?/",
    r"[Ss]ervices?/",
    r"[Hh]andlers?/",
...
def calculate_cyclomatic_complexity(filepath: Path) -> int:
    ...
    content = re.sub(r"@""(?:[^""]|"""")*""", '""', content)  # verbatim strings
    content = re.sub(r"\$""(?:[^""]|"""")*""", '""', content)  # interpolated strings
...
def calculate_business_criticality(filepath: str,
    user_overrides: dict[str, str] | None = None,
    default_level: str = "medium",
) -> float:
    """Estimate business criticality. User overrides take precedence over path heuristics."""
    normalized = filepath.replace("\\", "/")

    # Check user-provided criticality overrides first
    if user_overrides:
        for pattern, level in user_overrides.items():
            norm_pattern = pattern.replace("\\", "/")
            if fnmatch.fnmatch(normalized.lower(), norm_pattern.lower()):
    ...
    # Fall back to configured default level
...
def main():
    parser = argparse.ArgumentParser(
        description="Compute risk scores for C# source files."
    )
    parser.add_argument(
        "files_json",
    ...
    parser.add_argument(
        "--codebase", "-c",
        help="Codebase path (for git commands).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--weights",
        help='JSON string with factor weights',
    )
    parser.add_argument(
        "--criticality-map",
        help="Path to JSON file mapping file/namespace glob patterns to criticality levels",
    )
    ...
    if "impactedFiles" in data:
        files = [f["file"] for f in data["impactedFiles"] if not f.get("isTestFile", False)]
    ...
    if args.weights:
        try:
            ...
            print("Warning: Invalid weights JSON, using defaults.", file=sys.stderr)

    # Load user-provided criticality overrides
    ...
    if args.criticality_map:
        ...
        if crit_path.exists():
            try:
                ...
                # Support both flat format {"pattern": "level"} and nested
                # {"overrides": {...}, "default_level": "medium"}
    ...
    for m in raw_metrics:
        ...
        scored_files.append({
            ...
            "riskLevel": get_risk_level(composite),
            "factors": {
