#!/usr/bin/env python3
"""
analyze-changes.py — Parse git diff for changed JavaScript/TypeScript files.

Usage:
    python analyze-changes.py <codebase-path> [--ref HEAD~1] [--files file1,file2]
                              [--output <file>]

Detects changed JS/TS source files from git diff or a user-provided file list.
...
def is_js_ts_source(filepath: str) -> bool:
    """Check if a file is a JS/TS source file."""
    return Path(filepath).suffix in SOURCE_EXTENSIONS


def is_test_file(filepath: str) -> bool:
    """Check if a file is a JS/TS test file."""
    name = Path(filepath).name
    normalized = filepath.replace("\\", "/")
    if re.match(r".*\.(test|spec)\.(js|jsx|ts|tsx|mjs|cjs)$", name):
        return True
    if "/__tests__/" in normalized or "\\__tests__\\" in filepath:
        return True
    return False
...
def parse_git_diff(codebase_path: str, ref: str) -> list[dict[str, Any]]:
    ...
    for line in diff_stat.strip().split("\n"):
        ...
        if not is_js_ts_source(filepath):
            continue
...
def detect_build_file_changes(codebase_path: str, ref: str) -> list[str]:
    """Detect changes to build/config files."""
    diff_names = run_git_command(
...
def main():
    parser = argparse.ArgumentParser(
        description="Parse git diff for changed JavaScript/TypeScript files."
    )
