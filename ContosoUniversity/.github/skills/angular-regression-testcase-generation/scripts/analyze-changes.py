#!/usr/bin/env python3
"""
analyze-changes.py — Parse git diff for changed Angular/TypeScript files.

Usage:
    python analyze-changes.py <codebase-path> [--ref HEAD~1] [--files file1,file2]
                              [--output <file>]

Detects changed Angular source files from git diff or a user-provided file list.
...
# Angular/TypeScript source file extensions
SOURCE_EXTENSIONS = {".ts", ".html", ".scss", ".css"}

# Directories to exclude from change detection
EXCLUDE_DIRS = {
    "node_modules", ".git", ".svn", ".hg", "dist", "coverage",
...
def is_angular_source(filepath: str) -> bool:
    """Check if a file is an Angular/TypeScript source file."""
    return Path(filepath).suffix in SOURCE_EXTENSIONS


def is_test_file(filepath: str) -> bool:
    """Check if a file is an Angular test file."""
    name = Path(filepath).name
    return bool(re.match(r".*\.spec\.ts$|.*\.test\.ts$|.*\.integration\.spec\.ts$", name))
...
def parse_git_diff(codebase_path: str, ref: str) -> list[dict[str, Any]]:
    ...
    # Parse name-status for change type
    ...
    # Parse numstat for lines added/removed
    ...
    for line in diff_stat.strip().split("\n"):
        ...
        if not is_angular_source(filepath):
            continue
...
def detect_build_file_changes(codebase_path: str, ref: str) -> list[str]:
    """Detect changes to build configuration files."""
    diff_names = run_git_command(
...
def main():
    parser = argparse.ArgumentParser(
        description="Parse git diff for changed Angular/TypeScript files."
    )
    ...
    # Separate source and test changes
