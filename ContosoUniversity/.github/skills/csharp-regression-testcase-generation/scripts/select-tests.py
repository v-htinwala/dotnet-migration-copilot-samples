...
new test generation. Identical logic to Java/JS versions — language-agnostic scoring.
"""
...
DEFAULT_CONFIG = {
    "change_based": {"enabled": True, "weight": 0.4},
    "risk_based": {"enabled": True, "weight": 0.35, "min_risk_level": "medium"},
    "coverage_based": {"enabled": True, "weight": 0.25, "generate_for_uncovered": True},
}

RISK_VALUES = {"critical": 100, "high": 75, "medium": 50, "low": 25}
...
def calculate_change_relevance(filepath: str, changes_data: dict[str, Any]) -> float:
...
def calculate_impact_relevance(filepath: str, impact_data: dict[str, Any]) -> float:
    for impacted in impact_data.get("impactedFiles", []):
        if impacted["file"] == filepath:
            depth = impacted.get("depth", 0)
            if depth == 0: return 100.0
            elif depth == 1: return 70.0
            elif depth == 2: return 40.0
            else: return max(10.0, 100 - depth * 30)
    return 0.0


def combined_score(filepath, changes_data, impact_data, risk_data, config,
...
def select_tests(test_map, scores, risk_data, config, max_tests=None):
...
def main():
    ...
    scores = [combined_score(fp, changes_data, impact_data, risk_data, config,
                             scenarios, scenario_boost_points) for fp in all_sources]
    selected_tests, coverage_gaps = select_tests(test_map, scores, risk_data, config, args.max_tests)
