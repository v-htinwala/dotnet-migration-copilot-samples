#!/usr/bin/env python3
"""
analyze-changes.py — Parse git diff for changed Java files.

Usage:
    python analyze-changes.py <codebase-path> [--ref HEAD~1] [--files file1,file2]
                              [--output <file>]

Detects changed Java source files from git diff or a user-provided file list.
...
EXCLUDE_DIRS = {
    "node_modules", ".git", ".svn", ".hg", "target", "build", "out",
...
def is_java_source(filepath: str) -> bool:
...
def is_test_file(filepath: str) -> bool:
    ...
    return bool(re.match(r".*Test\.java$|.*Tests\.java$|.*IT\.java$|.*Spec\.java$", name))
...
def parse_git_diff(codebase_path: str, ref: str) -> list[dict[str, Any]]:
    ...
    for line in diff_stat.strip().split("\n"):
        ...
        if not is_java_source(filepath):
            continue
...
def main():
    parser = argparse.ArgumentParser(
        description="Parse git diff for changed Java files."
    )
