#!/usr/bin/env python3
"""
calculate-risk-scores.py — Compute risk scores for Java source files.
...
# Cyclomatic complexity keywords for Java
COMPLEXITY_KEYWORDS = [
    r"\bif\b",
    r"\belse\s+if\b",
    r"\bfor\b",
    ...
    r"\bcase\b",
    r"\bcatch\b",
    r"\?[^?]",  # Ternary operator (but not ??)
    r"&&",
    r"\|\|",
]

# Business criticality path heuristics for Java
CRITICALITY_HIGH = [
    r"controller[s]?/",
    r"api/",
    r"service[s]?/",
...
def normalize_score(value: float, max_value: float) -> float:
    ...
    if max_value <= 0:
        return 0.0
    # Log-based normalization to handle outliers
...
def main():
    parser = argparse.ArgumentParser(
        description="Compute risk scores for Java source files."
    )
    parser.add_argument(
        "files_json",
    ...
    for m in raw_metrics:
        ...
        norm_biz_crit = m["rawBusinessCriticality"] * 100  # Already 0-1, scale to 0-100
