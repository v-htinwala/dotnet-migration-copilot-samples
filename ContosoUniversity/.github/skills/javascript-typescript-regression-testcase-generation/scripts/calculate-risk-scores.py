#!/usr/bin/env python3
"""
calculate-risk-scores.py — Compute risk scores for JavaScript/TypeScript source files.
...
# Cyclomatic complexity keywords for JS/TS
COMPLEXITY_KEYWORDS = [
    r"\bif\b",
    r"\belse\s+if\b",
    r"\bfor\b",
    ...
    r"\bcase\b",
    r"\bcatch\b",
    r"\?[^?.:]",  # Ternary operator (not ?. or ??)
    r"&&",
    r"\|\|",
    r"\?\?",     # Nullish coalescing
    r"\?\.",     # Optional chaining (adds branching)
]

# Business criticality path heuristics for JS/TS
CRITICALITY_HIGH = [
    r"routes?/",
    r"api/",
    r"controllers?/",
    ...
    r"payment/",
    r"gateway/",
    r"pages/api/",     # Next.js API routes
    r"server/",
    r"graphql/",
]
...
def calculate_cyclomatic_complexity(filepath: Path) -> int:
    ...
    content = re.sub(r'`(?:[^`\\]|\\.)*`', '""', content, flags=re.DOTALL)  # template literals
...
def main():
    parser = argparse.ArgumentParser(
        description="Compute risk scores for JavaScript/TypeScript source files."
    )
    parser.add_argument(
        "files_json",
