...
This document applies equally to Playwright E2E and component tests. The criteria are framework-agnostic — they operate on source file analysis, not test runner specifics.
...
2. Build a dependency graph from ESM `import` and CJS `require()` analysis
3. Resolve imports via `tsconfig.json` `paths` aliases and monorepo workspace packages
4. Traverse dependents up to `impact_depth` levels (default: 2)
5. Map each impacted file to its existing Playwright test file(s)
...
**Process**:
1. Parse existing coverage reports (Istanbul `lcov.info`, `coverage-summary.json`, or Playwright coverage API output)
2. Filter to changed/impacted files only
3. Identify functions/branches with zero or low coverage
4. Flag uncovered items for new test generation

**Note on Playwright coverage**: Playwright can collect V8 code coverage via `page.coverage` API for Chromium-based browsers. Alternatively, use Istanbul-instrumented builds (via `nyc` or `babel-plugin-istanbul`) for framework-agnostic coverage. The coverage data is then analyzed identically to Jest/Vitest coverage.
...
```json
{
  "qualification": {
    ...
    "risk_based": {
      ...
      "business_criticality": {
        "overrides": {
          "src/features/payment/**": "critical",
          "src/hooks/useAuth.ts": "critical",
          "src/api/**": "high",
          "src/context/CartContext.tsx": "high"
        },
  ...
  "exclusions": {
    "directories": ["node_modules", ".git", "dist", "build", ".next", "coverage", "out"],
    "file_patterns": ["*.d.ts", "*.generated.*", "next-env.d.ts", "*.config.js", "*.config.ts"]
  },
  "execution": {
    "batch_size": 5,
    "max_retries": 3,
    ...
    "report_formats": ["junit-xml", "html", "json"],
    "test_splitting": {
      "enabled": false,
      "total_shards": 1
    },
