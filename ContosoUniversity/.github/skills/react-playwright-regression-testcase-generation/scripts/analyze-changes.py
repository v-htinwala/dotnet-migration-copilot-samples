#!/usr/bin/env python3
"""
analyze-changes.py — Parse git diff for changed React/TypeScript/JavaScript files.

Usage:
    python analyze-changes.py <codebase-path> [--ref HEAD~1] [--files file1,file2]
                              [--output <file>]

Detects changed React/TypeScript/JavaScript source files from git diff or a
user-provided file list. Outputs changes.json with changed files and diff stats.

This script is identical to the Jest/Vitest variant — change detection is
framework-agnostic.
"""
...
# React/TypeScript/JavaScript source file extensions
...
EXCLUDE_DIRS = {
    ...
    "out", "coverage", ".cache", ".turbo", ".vercel", ".parcel-cache",
    "__snapshots__",
}
...
def is_source_file(filepath: str) -> bool:
    """Check if a file is a React/TypeScript/JavaScript source file."""
    return Path(filepath).suffix in SOURCE_EXTENSIONS


def is_test_file(filepath: str) -> bool:
    """Check if a file is a test file (supports Playwright conventions)."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Parse git diff for changed React/TypeScript/JavaScript files."
    )
