#!/usr/bin/env python3
"""
select-tests.py — Apply qualification criteria, rank, and select Playwright regression tests.
...
prioritized list of existing Playwright tests to run, and identifies coverage
gaps needing new test generation.

This script is framework-agnostic — identical logic applies across test runners.
Uses Playwright's shard format (N/M) instead of separate index/total args.
"""
    ...
    for mapping in test_map.get("mappings", []):
        ...
        if has_test and test_file:
            selected_tests.append({
                ...
                "testFile": test_file,
                "testType": mapping.get("testType", "e2e"),
            ...
            if config["coverage_based"].get("generate_for_uncovered", True):
                coverage_gaps.append({
                    ...
                    "priority": final_score,
                    "riskLevel": risk_level,
                    "reason": f"No existing Playwright test for {source_file}",
                })
...
def parse_shard(shard_str: str) -> tuple[int, int]:
    """Parse Playwright shard format 'N/M' into (index, total)."""
    parts = shard_str.split("/")
    if len(parts) != 2:
        raise ValueError(f"Invalid shard format: {shard_str}. Expected N/M.")
    return int(parts[0]), int(parts[1])


def main():
    parser = argparse.ArgumentParser(
        description="Apply qualification criteria, rank, and select Playwright regression tests."
    )
    ...
    # Apply test splitting (sharding) using Playwright format
    shard_info = None
    if args.shard:
        shard_index, total_shards = parse_shard(args.shard)
        total_shards = max(1, total_shards)
        # Playwright uses 1-based shards; convert to 0-based for partitioning
        zero_idx = (shard_index - 1) % total_shards
        selected_tests = [
            t for i, t in enumerate(selected_tests) if i % total_shards == zero_idx
        ]
        shard_info = {
            "current": shard_index,
            "total": total_shards,
