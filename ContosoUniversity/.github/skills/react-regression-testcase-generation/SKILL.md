---
name: react-regression-testcase-generation
description:  Generates regression test cases for React codebases. Selects existing
  tests and generates new tests based on change-based, risk-based, and coverage-based
  qualification criteria. Supports React projects with Jest or Vitest
  (auto-detected from package.json/vite.config.*). Generates both unit-level
  (render, screen, userEvent, jest.mock/vi.mock) and integration-level (MSW,
  MemoryRouter, full-app rendering) regression tests. Supports Next.js App Router
  detection for special handling. Supports user-supplied test data (fixtures,
  JSON seeds), configurable business criticality, user-defined regression scenarios
  with requirement traceability, and CI/CD integration with artifact generation,
  ...
  React projects.
license: Apache-2.0
compatibility:  Best with VS Code Copilot or agents supporting the runSubagent tool for parallel
  processing. Falls back to sequential file-by-file processing on other
  platforms. Requires Python 3.8+ for helper scripts. Node.js 16+ and React 17+
  must be installed.
metadata:
...
# React Regression Test Case Generation

## When to Use This Skill

Activate this skill when the user wants to:
- Generate regression tests for a React codebase based on recent code changes
...
- Analyze the impact of changes across a React project (components, hooks, context, API routes)
...
| **Test framework** | No | auto-detect | Jest or Vitest (auto-detected from `package.json` dependencies / `vite.config.*` / `jest.config.*`) |
| **Mock library** | No | auto-detect | `jest.mock`/`jest.fn` (Jest) or `vi.mock`/`vi.fn` (Vitest) |
...
| **Test level** | No | `unit` | `unit` (render, screen, userEvent) or `integration` (MSW, MemoryRouter, full-app) |
| **Coverage format** | No | `istanbul` | Istanbul coverage via Jest or Vitest (c8/istanbul) |
...
2. **NEVER alter existing test methods.** When adding to an existing test file,
   only append new `describe`/`test` blocks — do not change existing ones.
3. **All file writes MUST target test files only.** Valid patterns:
   - `*.test.tsx`, `*.test.ts` (React convention)
   - `*.spec.tsx`, `*.spec.ts` (alternative convention)
...
- Detect test framework: check `package.json` for `jest` or `vitest` dependency; check for `jest.config.*` or `vitest.config.*`/`vite.config.*`
...
- If `test_level` is `integration` or `both`, verify `msw` and `@testing-library/react` are available
...
- Path heuristics: `**/hooks/**` (high), `**/context/**` (high), `**/api/**` (high), `**/pages/**` (high), `**/components/**` (medium), `**/utils/**` (low), `**/types/**` (low)
...
- Each receives: source file, component/hook metadata, gap details, React testing reference, test framework (Jest/Vitest), test level, matching fixtures, matching regression scenarios, requirement IDs
- For `test_level=unit`: generate tests using `render()`, `screen`, `userEvent`, `jest.mock()`/`vi.mock()`
- For `test_level=integration`: generate tests using MSW handlers, `MemoryRouter`, full-app rendering with providers
- For `test_level=both`: generate separate unit and integration test blocks in `*.test.tsx` and `*.integration.test.tsx` files
- When user-provided fixture files match the target entity, generator loads fixture data via `import` or inline `test.each`
...
- When requirement IDs are provided, generator adds `// REQ-xxx` comments
- Write test files following project convention (co-located or `__tests__/`)
...
6. For each coverage gap (one at a time):
   ...
   d. Write the test file as `*.test.tsx`
...
|---|---|---|
| `UserProfile.tsx` | `UserProfile.test.tsx` | `UserProfile.integration.test.tsx` |
| `useAuth.ts` | `useAuth.test.ts` | `useAuth.integration.test.ts` |
| `api/users.ts` | `api/users.test.ts` | `api/users.integration.test.ts` |
| `CartContext.tsx` | `CartContext.test.tsx` | `CartContext.integration.test.tsx` |
...
**Vitest**:
```bash
npx vitest run --coverage --reporter=junit --outputFile=regression-reports/regression-results.xml
```
...
| `list-existing-tests.py` | Map React source files to test files | `<module-path>` | `test-map.json` (includes `testType: unit&#124;integration`) |
...
| `run-regression-tests.py` | Execute selected tests via jest/vitest | `<module> --test-runner <jest&#124;vitest> [--ci-mode] [--shard-index N --total-shards M] [--flaky-threshold N]` | `results.json` |
| `check-coverage.py` | Istanbul/c8 coverage gaps on changed code | `<report> --threshold <N>` | `coverage-gaps.json` |

## Language-Specific Reference

Load the React regression patterns reference when generating tests:

- [React Regression Patterns](references/regression-patterns.md)
