#!/usr/bin/env python3
"""
Video Frame Extractor (Regression)
===================================
Extracts frames (screenshots) from a video file and saves them as individual images.
Tuned for regression test case generation with moderate sensitivity defaults.

This is the same extraction engine as the UAT variant. The SKILL.md recommends
different default parameters for regression use cases.

Usage:
    python extract_frames.py --input video.mp4 --output-dir ./frames --scene-detect
    python extract_frames.py --input video.mp4 --output-dir ./frames --scene-detect --threshold 0.85
    python extract_frames.py --input video.mp4 --output-dir ./frames --scene-detect --threshold 0.95
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Extract frames from a video file for regression test case generation.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Examples:\n"
            "  %(prog)s --input video.mp4 --output-dir ./frames --scene-detect\n"
            "  %(prog)s --input video.mp4 --output-dir ./frames --scene-detect --threshold 0.85\n"
            "  %(prog)s --input video.mp4 --output-dir ./frames --scene-detect --threshold 0.95\n"
        ),
    )
    parser.add_argument(
        "--input",
    ...
    parser.add_argument(
        ...
        choices=["png", "jpg"],
        default="png",
        help="Output image format. Default: png (lossless for text clarity).",
    )
    parser.add_argument(
        "--quality",
        type=int,
    ...
    parser.add_argument(
        ...
        help="Similarity threshold for scene detection (0.0-1.0). Frames with "
             ...
             "Lower = fewer frames, higher = more frames. Default: 0.85.",
    )
    parser.add_argument(
        "--min-interval",
        type=int,
...
def extract_frames(
    ...
    print("=" * 60)
    print("Video Frame Extractor (Regression)")
    print("=" * 60)
    ...
    print(f"  Format      : {fmt}")
    if scene_detect:
        print(f"  Mode        : scene-detect (regression)")
